...
WHERE Name LIKE '%Jack%'
   OR '%John%'
    AND Name NOT LIKE (SELECT Name
                       FROM Artist
                       WHERE Name IN ('Martin'));
...
# 5. Show the information, without repeated records, of all albums
# that feature songs of the “Bossa Nova” genre whose title starts by the word “Samba”.
SELECT *
FROM Genre;
...
# One Company has no state
SELECT *
FROM Customer
WHERE Company IS NOT NULL
  AND State IS NULL;

# 8. For each employee with clients in the “USA”, “Canada” and “Mexico” show
# the number of clients from these countries s/he has given support, only when this number is higher than 6.
